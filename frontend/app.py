...
with st.sidebar:
    st.header("⚙️ Model Tuning Parameters")
...
with col1:
    ...
    cat_name = st.text_input("🏷️ Department Category Classification", value="Electronics", key="c_name_input")
...
with col2:
    ...
    if st.button("🚀 Execute Content Generation Pipeline", use_container_width=True):
        if not prod_name.strip() or not cat_name.strip():
            st.warning("⚠️ Action Required: Populate minimum descriptive validation variables (Name & Category).")
        else:
